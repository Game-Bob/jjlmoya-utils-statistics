...
export interface StatisticsToolEntry<TUI extends Record<string, string> = Record<string, string>> {
  id: string;
  icons: {
    bg: string;
    fg: string;
...
export interface StatisticsCategoryEntry {
  icon: string;
  tools: StatisticsToolEntry[];
  i18n: LocaleMap<CategoryLocaleContent>;
}

export interface ToolDefinition {
  entry: StatisticsToolEntry;
  Component: unknown;
